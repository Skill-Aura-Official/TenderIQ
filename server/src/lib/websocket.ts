...
export function setupWebSockets(app: FastifyInstance) {
  ...
  app.get('/api/v1/ws', { websocket: true }, async (ws: WebSocket, req: FastifyRequest) => {
    try {
 ...
 */
export function broadcastToOrg(orgId: string, eventType: string, payload: any = {}) {
  const orgConnections = activeConnections.get(orgId);
  if (!orgConnections) return; // No active connections for this org
