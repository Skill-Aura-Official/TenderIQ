...
export function calculateScore(profile: any, tender: any) {
  const currentYear = new Date().getFullYear();
  const companyAge = profile.incorporationYear ? (currentYear - profile.incorporationYear) : 0;
  
  // Parse JSON columns
  const profileServices = JSON.parse(profile.services || '[]') as string[];
  const profileCerts = JSON.parse(profile.certifications || '[]') as string[];
  const profileStates = JSON.parse(profile.operatingStates || '[]') as string[];
  
  const tenderCategories = JSON.parse(tender.categoryCodes || '[]') as string[];
  const tenderStates = JSON.parse(tender.stateCodes || '[]') as string[];
  
  let tenderCriteria: any[] = [];
  try {
  ...
  const serviceOverlap = profileServices.filter(service => 
    tenderCategories.map(c => normalize(c)).includes(normalize(service)) ||
  ...
  const requiredCerts = tenderCriteria
    .filter((c: any) => c.criterion === 'certification')
    .map((c: any) => c.value);
  ...
  if (requiredCerts.length > 0) {
    const matchedCerts = requiredCerts.filter((c: string) => profileCerts.includes(c));
    ...
    requiredCerts.forEach((c: string) => {
  ...
  const expCriterion = tenderCriteria.find((c: any) => c.criterion === 'incorporation_years');
  let expPoints = 20;
  let expMatched = true;
...
export async function recalculateScoresForNewTender(tenderId: string, orgId: string) {
