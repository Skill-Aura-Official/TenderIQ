...
export default async function tenderRoutes(fastify) {
    ...
    fastify.get('/tenders', async (request, reply) => {
        ...
        try {
            ...
            if (query.category) {
                results = results.filter((t) => t.categoryCodes.includes(query.category));
            }
            // Apply state filter
            if (query.state) {
                results = results.filter((t) => t.stateCodes.includes(query.state) || t.stateCodes.includes('ALL'));
            }
            // Apply portal filter
            if (query.portal) {
                results = results.filter((t) => t.portalSlug === query.portal);
            }
            // Apply match score grade filter
            if (query.scoreGrade) {
                results = results.filter((t) => {
                    if (query.scoreGrade === 'excellent')
                        return t.matchScore >= 90;
                    if (query.scoreGrade === 'good')
                        return t.matchScore >= 70 && t.matchScore < 90;
                    if (query.scoreGrade === 'moderate')
                        return t.matchScore >= 50 && t.matchScore < 70;
                    if (query.scoreGrade === 'poor')
                        return t.matchScore < 50;
                    return true;
                });
            }
            // Apply value filters
            if (query.minValue) {
                results = results.filter((t) => (t.estimatedValue || 0) >= parseInt(query.minValue, 10));
            }
            if (query.maxValue) {
                results = results.filter((t) => (t.estimatedValue || 0) <= parseInt(query.maxValue, 10));
            }
            ...
            if (sort === 'score') {
                ...
                results.sort((a, b) => a.submissionDeadline - b.submissionDeadline);
            }
            else if (sort === 'value') {
